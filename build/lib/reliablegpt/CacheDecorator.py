...
class reliableCache: 
    ...
    def print_verbose(self, print_statement):
        if self.verbose:
            print(f"Cached Request: {str(print_statement)}")
    ...
    def try_cache_request(self, user_email, instance_id, query=None):
        try:
            if (user_email, instance_id, query) in self.hot_cache:
                return self.hot_cache[(user_email, instance_id, query)]
    ...
    def get_wrapper_thread_utilization(self):
        ...
        active_cache_threads = sum(
            1 for value in self.cache_wrapper_threads.values() if value == True
        )
